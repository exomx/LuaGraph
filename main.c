...
int show_warnings_screen = 1, show_warnings_console = 1, show_errors_screen = 1, show_errors_console = 1;
...
static int LUAPROC_DrawQuadFly(lua_State* L) { //fly meaning "on the fly"
    ...
    else if(show_warnings_console || show_warnings_screen)
        DEBUG_Err("draw_quadfast", "Texture was NIL, assuming no texture", "Texture attribute in rect table was NIL, assumed no texture (0)", 1);

    if (lua_type(L, 10) == LUA_TNUMBER)
        angle = luaL_checknumber(L, 10);
    else if (show_warnings_console || show_warnings_screen)
        DEBUG_Err("draw_quadfast", "Angle was NIL, assuming angle 0", "Angle attribute in rect table was NIL, assumed angle of 0", 1);
...
static int LUAPROC_DrawQuadFlyST(lua_State* L) { //fly meaning "on the fly"
    ...
    if (lua_type(L, 10) == LUA_TNUMBER)
        ...
        DEBUG_Err("draw_quadfastsheet", "Texture was NIL, assuming no texture", "Texture attribute in rect table was NIL, assumed no texture (0)", 1);

    if (lua_type(L, 11) == LUA_TNUMBER)
        angle = luaL_checknumber(L, 11);
    else if (show_warnings_console || show_warnings_screen)
        DEBUG_Err("draw_quadfastsheet", "Angle was NIL, assuming angle 0", "Angle attribute in rect table was NIL, assumed angle of 0", 1);
    //source rect
    ...
    if (lua_type(L, 12) == LUA_TNUMBER)
        ...
        DEBUG_Err("draw_quadfastsheet", "Source rect x was NIL, assuming 0", "x attribute in source rect table was NIL, assumed x of 0", 1);

    if (lua_type(L, 13) == LUA_TNUMBER)
        ty = luaL_checknumber(L, 13);
    else if (show_warnings_console || show_warnings_screen)
        DEBUG_Err("draw_quadfastsheet", "Source rect y was NIL, assuming 0", "y attribute in source rect table was NIL, assumed y of 0", 1);
...
static int LUAPROC_SetDrawBuffer(lua_State* L) {
    ...
    if (lua_type(L, -1) == LUA_TNUMBER)
        ...
        drawbuf_tex = 0, DEBUG_Err("set_glbuffer", "Texture was NIL, assuming no texture", "Texture attribute in glbuffer table was NIL, assumed no texture (0)", 1);
    ...
    for (int i = 0; i < amount_of_quads; ++i) {
        ...
        if (lua_type(L, -1) == LUA_TNUMBER)
            ...
            DEBUG_Err("set_glbuffer", "Angle was NIL, assuming 0", "Angle in attribute was NIL in rect in glbuffer, assumed 0", 1);
...
static int LUAPROC_ClearWindow(lua_State* L) {
    ...
    starttime = clock(); //very rought estimate for start of frame
